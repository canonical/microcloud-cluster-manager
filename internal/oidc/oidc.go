...
func (o *Verifier) Auth(ctx context.Context, r *http.Request) (*AuthenticationResult, response.Response, error) {
	...
	if err != nil {
		...
		resp = response.ManualResponse(func(w http.ResponseWriter) error {
			_ = o.setCookies(w, nil, uuid.UUID{}, "", "", true)
			return response.Forbidden(err).Render(w)
		})

		return nil, resp, fmt.Errorf("Failed to retrieve login information: %w", err)
	}

	if idToken == "" && refreshToken == "" {
		return nil, nil, fmt.Errorf("No ID or refresh tokens present in request")
	}

	// When authenticating via the UI, we expect that there will be ID and refresh tokens present in the request cookies.
	result, resp, err := o.authenticateIDToken(ctx, idToken, refreshToken)
	if err != nil {
		return nil, resp, err
	}
...
func (o *Verifier) authenticateIDToken(ctx context.Context, idToken string, refreshToken string) (result *AuthenticationResult, resp response.Response, e error) {
	...
	if err != nil {
		return nil, nil, AuthError{Err: fmt.Errorf("Failed to refresh ID tokens: %w", err)}
	}

	idTokenAny := tokens.Extra("id_token")
	if idTokenAny == nil {
		return nil, nil, AuthError{Err: errors.New("ID tokens missing from OIDC refresh response")}
	}

	idToken, ok := idTokenAny.(string)
	if !ok {
		return nil, nil, AuthError{Err: errors.New("Malformed ID tokens in OIDC refresh response")}
	}

	// Verify the refreshed ID token.
	claims, err = rp.VerifyIDToken[*oidc.IDTokenClaims](ctx, idToken, o.relyingParty.IDTokenVerifier())
	if err != nil {
		return nil, nil, AuthError{Err: fmt.Errorf("Failed to verify refreshed ID token: %w", err)}
	}

	resp = response.ManualResponse(func(w http.ResponseWriter) error {
		err := o.WriteTokenToCookies(w, idToken, tokens.RefreshToken)
		if err != nil {
			return response.InternalError(err).Render(w)
		}
...
func NewVerifier(issuer string, clientID string, audience string, clusterCert func() *shared.CertInfo, httpClientFunc func() (*http.Client, error)) (*Verifier, error) {
	verifier := &Verifier{
		issuer:               issuer,
		clientID:             clientID,
		audience:             audience,
