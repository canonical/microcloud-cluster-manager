...
func registerEndpoint(mux *mux.Router, prefix string, e types.Endpoint, rc types.RouteConfig) {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !e.AllowUnauthorized {
			err := rc.Auth.Authorizor.CheckPermissions(r.Context(), e.AllowedEntitlements)
			if err != nil {
				logger.Log.Errorw("AUTHZ authorization failed", "path", path.Join(prefix, e.Path), "ERROR", err)
				renderErr := response.ErrorResponse(http.StatusForbidden, "Permission denied").Render(w, r)
				if renderErr != nil {
					logger.Log.Errorw("failed to write error response", "path", path.Join(prefix, e.Path), "ERROR", renderErr.Error())
				}
				return
