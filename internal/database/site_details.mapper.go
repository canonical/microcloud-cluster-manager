...
SELECT site_details.core_site_id, site_details.status, site_details.id, site_details.instance_count, site_details.instance_statuses, site_details.joined_at, site_details.updated_at
  FROM site_details
...
INSERT INTO site_details (core_site_id, status, instance_count, instance_statuses, joined_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?)
`)

var siteDetailUpdate = cluster.RegisterStmt(`
UPDATE site_details
  SET core_site_id = ?, status = ?, instance_count = ?, instance_statuses = ?, joined_at = ?, updated_at = ?
 WHERE id = ?
`)
...
func siteDetailColumns() string {
	return "sites_details.core_site_id, sites_details.status, sites_details.id, sites_details.instance_count, sites_details.instance_statuses, sites_details.joined_at, sites_details.updated_at"
}
...
func getSiteDetails(ctx context.Context, stmt *sql.Stmt, args ...any) ([]SiteDetail, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		s := SiteDetail{}
		err := scan(&s.CoreSiteID, &s.Status, &s.ID, &s.InstanceCount, &s.InstanceStatuses, &s.JoinedAt, &s.UpdatedAt)
		if err != nil {
			return err
		}
...
func CreateSiteDetail(ctx context.Context, tx *sql.Tx, object SiteDetail) (int64, error) {
	...
	args[2] = object.InstanceCount
	args[3] = object.InstanceStatuses
	args[4] = object.JoinedAt
	args[5] = object.UpdatedAt
...
func UpdateSiteDetail(ctx context.Context, tx *sql.Tx, coreSiteID int, status string, object SiteDetail) error {
	...
	result, err := stmt.Exec(object.CoreSiteID, object.Status, object.InstanceCount, object.InstanceStatuses, object.JoinedAt, object.UpdatedAt, id)
	if err != nil {
