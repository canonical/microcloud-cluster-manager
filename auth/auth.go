...
const (
	OidcIssuer   = "https://dev-xjrvvfikbsv4jxn7.us.auth0.com/"
	OidcClientID = "gxj297yfAjmklILK5WqPWDSbtVBAeSQm"
	OidcAudience = "https://dev-xjrvvfikbsv4jxn7.us.auth0.com/api/v2/"
	HttpsProxy   = "https://0.0.0.0:8414"
)
...
func (o *Verifier) Callback(w http.ResponseWriter, r *http.Request) {
	// TODO: callback is hitting an authorisation error, probably
	...
	handler := rp.CodeExchangeHandler(func(w http.ResponseWriter, r *http.Request, tokens *oidc.Tokens[*oidc.IDTokenClaims], state string, rp rp.RelyingParty) {
		...
		http.Redirect(w, r, "/1.0/ui/", http.StatusMovedPermanently)
	}, o.relyingParty)
...
func (o *Verifier) setRelyingParty(host string) error {
	...
	// TODO: how can we get host as the proxy url and not the member url?
	relyingParty, err := rp.NewRelyingPartyOIDC(o.issuer, o.clientID, "", fmt.Sprintf("%s/1.0/oidc/callback", HttpsProxy), oidcScopes, options...)
	if err != nil {
...
func GetOIDCVerifierFromContext(ctx context.Context) (*Verifier, error) {
	oidcVerifierFromContext := ctx.Value("oidcVerifier")
	if oidcVerifierFromContext == nil {
		return nil, errors.New("OIDC verifier not found in context")
	}

	oidcVerifier, ok := oidcVerifierFromContext.(*Verifier)
	if !ok {
		return nil, errors.New("OIDC verifier is not of the correct type")
	}
